...
- S11 captures SmartDevice-Handheld BLE ICD v0.13 as a versioned wire contract with opcode table, return codes, UUIDs, frame encode/decode, CRC-16/X25, measurement/calibration rules, and recovery subset.
...
| `docs/test-evidence/S11-pytest.txt` | Local pytest output for 56 passed tests and 3 intentionally skipped target-board HIL tests. |
...
| `tests/test_icd_v013.py` | Verifies v0.13 metadata, opcodes, return codes, measurement rules, recovery subset, and frame CRC behavior. |
